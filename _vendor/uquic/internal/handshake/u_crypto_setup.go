...
func NewUCryptoSetupClient(
	...
	postApply func(*tls.UQUICConn) error,
) CryptoSetup {
	cs := newUCryptoSetup(
		connID,
		tp,
	...
	// [UQUIC PATCH] Run any post-build hook (e.g. mirage's legacy_session_id injection).
	if postApply != nil {
		if err := postApply(cs.conn); err != nil {
			panic(err)
		}
	}
...
func (h *uCryptoSetup) handleEvent(ev tls.QUICEvent) (err error) {
	...
	case tls.QUICStoreSession:
		// [UQUIC PATCH for utls 1.8.2 compat] *UQUICConn no longer
		// exposes StoreSession in upstream utls. Mirage does not use
		// 0-RTT session resumption, so we silently drop the event.
		return nil
