// mirage-client is a SOCKS5 frontend that tunnels TCP traffic through
// a mirage QUIC connection. The QUIC layer is provided by the
// github.com/valtrogen/mirage/client package (a thin wrapper over
// uquic / quic-go); SOCKS5 framing lives entirely in this binary.
package main
	...
	for {
		...
		go func(c net.Conn) {
			...
			if err := handleSOCKS(ctx, mirageConn, c, logger); err != nil {
				logger.Warn("socks session", "err", err)
			}
		}(c)
	}
	wg.Wait()
