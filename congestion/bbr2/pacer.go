...
func (p *pacer) Budget(now time.Time) congestion.ByteCount {
	...
	added := adjustedRate.BytesInDuration(delta)
...
func (p *pacer) maxBurstSize() congestion.ByteCount {
	...
	bwBurst := pacingRate.Mul(1.25).BytesInDuration(minDelay)
...
// may be sent. The zero time.Time means "send now".
func (p *pacer) TimeUntilSend() time.Time {
	...
	adjustedRate := pacingRate.Mul(1.25)
	bytesPerSecond := adjustedRate.BytesPerSecond()
	if bytesPerSecond == 0 {
		return time.Time{}
	}
