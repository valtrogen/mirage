...
// TimeUntilSend returns the absolute time at which the next packet
// may be sent. When the connection is at or above the congestion
// window the call returns the package-private neverSendTime sentinel,
// which the boundary adapter translates into the public
// "send forbidden" return on the Controller interface.
func (s *BBR2Sender) TimeUntilSend(bytesInFlight congestion.ByteCount) time.Time {
	if bytesInFlight >= s.GetCongestionWindow() {
		return neverSendTime
	}
...
func (s *BBR2Sender) OnCongestionEventEx(
	priorInFlight congestion.ByteCount,
	eventTime time.Time,
	ackedPackets []congestion.AckedPacket,
	lostPackets []congestion.LostPacket,
) {
